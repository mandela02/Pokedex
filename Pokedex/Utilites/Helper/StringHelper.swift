...
class StringHelper {
    ...
    static func getEnglishText(from texts: [FlavorTextEntry]) -> String {
        guard let text = texts.filter({$0.language.name == "en"}).map({$0.flavorText}).uniques.first else {
            return ""
        }
    ...
    static func getEnglishText(from texts: [Name]) -> String {
        guard let text = texts.filter({$0.language.name == "en"}).map({$0.name}).uniques.randomElement() else {
            return ""
        }
        return text.replacingOccurrences(of: "\n", with: " ", options: .literal, range: nil)
    }
    ...
    static func getMachineId(from url: String) -> Int {
        let base = Constants.baseMachineUrl
        return Int(url.replacingOccurrences(of: base, with: "")
                    .replacingOccurrences(of: "/", with: "")) ?? 0
    }
